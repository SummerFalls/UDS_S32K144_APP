...
typedef struct {
    ...
    uint8_t *pStartFifoAddr; /*start fifo addr*/
...
/*********************Static value define**********************/
static uint8_t gs_ucFifo[TOTAL_BYTES] = {0};           /*total fifo len*/
...
void ApplyFifo(tLen i_xApplyFifoLen, tLen i_xFifoId, tErroCode *o_peApplyStatus)
{
    ...
    if ((i_xApplyFifoLen + STRUCT_LEN) > gs_xCleanFifoLen) {
    ...
    pstNode->xReadAddr = 0u;
    pstNode->pStartFifoAddr = (uint8_t *)((tFifoInfo *)(&gs_ucFifo[TOTAL_BYTES - gs_xCleanFifoLen]) + 1u);
    pstNode->eFifoStatus = FIFO_EMPTY;

    xNodeNeedSpace = (tLen)((uint8_t *)((tFifoInfo *)(&gs_ucFifo[TOTAL_BYTES - gs_xCleanFifoLen]) + 1u) -
                     ...
                     tErroCode *o_peWriteStatus)
{
    ...
    if ((uint8_t *)0u == i_pucWriteDataBuf) {
                      ...
                      tErroCode *o_peReadStatus)
{
    ...
    if ((uint8_t *)0u == o_pucReadDataBuf ||
            (tLen *)0u == o_pxReadLen ||
